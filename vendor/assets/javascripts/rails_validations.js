    ...
    'ActionView::Helpers::FormBuilder': {
      add: function(element, settings, message) {
        if (element.data('valid') !== false) {
          var wrapper = element.closest('.control-group');
          //wrapper.removeClass('success');
          wrapper.addClass('error');
          if (jQuery('label.help-inline[for="' + element.attr('id') + '"]')[0] == undefined){
            var messsage_block = $('<label class="help-inline" for="'+element.attr('id')+'"></label>');
            messsage_block.insertAfter(element);
          }
        }
        jQuery('label.help-inline[for="' + element.attr('id') + '"]').text(message);
      },
      remove: function(element, settings) {   
        ...
        if (jQuery('label.help-inline[for="' + element.attr('id') + '"]')[0] == undefined){
          var messsage_block = $('<label class="help-inline" for="'+element.attr('id')+'"></label>');
          //wrapper.append(messsage_block);
          messsage_block.insertAfter(element);
        }
        if(wrapper.hasClass('warning') && wrapper.find('.warning-message').length > 0){
          jQuery('label.help-inline[for="' + element.attr('id') + '"]').text(wrapper.find('.warning-message').html());
        }else{
          jQuery('label.help-inline[for="' + element.attr('id') + '"]').text('');
        }
      }
    },
